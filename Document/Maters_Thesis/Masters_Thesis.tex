...
\begin{abstract}
本論文では近年話題の生成AIやChatGPTでおなじみ，機械学習による手法を用いた2次元イジング模型の磁気相転移の検出に関する研究をまとめた．\par
...
\begin{figure}[H]
  ...
  \end{center}
\end{figure}
次に，2次元イジング模型を考える．2次元イジング模型は2次元平面状に$L \times L$個の格子点が等間隔に配置された模型である．図\ref{2次元イジング模型格子構造}は2次元イジング模型の代表的な格子構造である．左は正方イジング模型とよばれるもので最も基本的な格子構造である．一般的に2次元イジング模型と書かれたら，正方イジング模型のことを指す．そのため本論文でも正方イジング模型を単にイジング模型とよぶことにする．右は三角イジング模型とよばれる．\par
ハミルトニアンは
\begin{equation}
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item $\beta_i<\beta_{\text{cr}}$の場合$d=1$(常磁性相)，$\beta_i>\beta_{\text{cr}}$の場合$d=0$(強磁性相)とする
    \item $(\bm{\sigma},d)$とする．
  \end{enumerate}
\end{enumerate}
...
それでは，$y_K = \text{softmax}(w_{Ka}^{(2)}x_a + b_K^{(2)})$で式\ref{(19)}と式\ref{(23)}を用いると，次のような出力が得られる．
\begin{equation}
  y_K = \text{softmax}(-(p+r)KE - 2\epsilon(\phi G(K) + pK))
\end{equation}
ここで，$\text{max}$関数に影響を与えないため，$K$に依存しない項は落とした．この式は，実際に$-2\epsilon (\phi G(K) + pK) = (p + r)F$が満たされる場合，$y_K^{\text{theory}}$の形をとる．理論的に予測された重みを図\ref{温度測定器2理論的重み}にプロットした．
