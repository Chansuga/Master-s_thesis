...
\begin{equation}
  w_{\beta 0}^{(2)} = -\phi G(\beta) - p\beta + q, \ \
...
\begin{figure}[H]
  \begin{center}
    ...
    \caption{青，緑の線はそれぞれ式(\ref{(23)})の$w_{\beta 0}^{(2)}$と$w_{\beta 2}^{(2)}$を表している．横軸は入力の2次元イジング配位の温度ラベルを表している．}
    \label{温度測定器2理論的重み}
  \end{center}
\end{figure}

\begin{align}
  w_{\beta 0}^{(2)} &= \frac{p+r}{2\epsilon}F(\beta) + q, \\
