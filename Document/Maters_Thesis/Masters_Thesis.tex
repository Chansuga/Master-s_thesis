...
この章では，機械学習，特にニューラルネットワークを用いたイジングモデルの磁気相転移温度の検出に関する研究についてまとめる．\par
...
ここから，相の分類器によるイジング模型の磁気相転移の検出について扱う．カラスクイラとメルコ―は，2016年の5月に"Machine learning phases of matter"を投稿し，後にNature Physics誌に掲載された．彼らの研究によってはじめてニューラルネットワークを用いて相転移が検出可能であることが示された．ここでは相の分類を行うこの学習済みモデルを相の分類器とよぶことにする．以下では，2次元正方イジング模型の相の分類器の作成についての説明と，作成した2次元正方イジング模型の相の分類器が2次元三角イジング模型の相分類にそのまま適用できることを説明する．\par
\subsection{データセットの作成}
ニューラルネットワークを用いて学習を行うにはまず，モデルの学習のためのデータセットを作成する必要がある．相分類器の作成には，スピン配位とその配位が強磁性相か常磁性相かを示す教師ラベルをセットにした教師ありデータを大量に用意する．\par
...
\end{align*}
そして，メトロポリス法を用いて各ラベルに対して，90個のスピン配位をサンプルした．つまり全データ数は1800になる．\par
\subsubsection*{三角イジング模型}
また，三角イジング模型に対しても同様な方法でデータを生成した．ただし，三角イジング模型では次の逆温度ラベルを用いた．
\begin{align*}
  \{\beta_i\} 
