...
\end{equation}
で表される．$E(\mu, \mu')$と$E(\mu)$は$n$のオーダーであるため，式(\ref{行列要素})の形式から，$n$が大きい場合，$\mathcal{P}$の固有値は一般に$e^n$のオーダーであることが予想される．$\lambda_{\text{max}}$を$\mathrm{P}$の最大固有値とすると，
\begin{equation}
...
\subsection{行列の直積}
ここで，$V_3, V_2, V'_1$による便利な方法を述べる前に，行列の積の概念を導入する．\par
...
\subsection{スピン行列}
ここで，$V'_1, V_2, V_3$を便利に表現できる特別な行列をいくつか紹介する．よく知られた3つの$2 \times 2$パウリスピン行列を$X, Y, Z$とする．
\begin{equation}
...
\subsection{行列$V'_1, V_2, V_3$}
式(\ref{V1})から，$\mathrm{V}'_1$が$n$個の同じ$2 \times 2$行列の直積で表せることがわかる．
\begin{equation}
  V_1'=\bm{a} \times \bm{a} \times \cdots \times \bm{a}
\end{equation}
ここで，
\begin{equation}
  \bra{s}\bm{a} \ket{s'}
...
\end{equation}
である．式(\ref{})を用いることで以下を得る
\begin{equation}
  \bm{a}
  =\sqrt{2 \sinh (2 \beta \epsilon)} e^{\theta X}
\end{equation}
ここで，
\begin{equation}
...
である．したがって
\begin{equation}
  \mathrm{V}_1'=[2 \sinh (2 \beta \epsilon)]^{\frac{1}{2}} e^{\theta X} \times e^{\theta X} \times \cdots \times e^{\theta X}
\end{equation}
\begin{equation}
  e^{\theta X} \times e^{\theta X} \times \cdots \times e^{\theta X}=e^{\theta \mathrm{X}_1} e^{\theta \mathrm{X}}{ }_2 \cdots e^{\theta \mathrm{X}_n}=e^{\theta\left(\mathrm{X}_1+\mathrm{X}_2+\cdots+\mathrm{X}_n\right)}
\end{equation}
\begin{align}
  \mathrm{V}_1'
   & =[2 \sinh (2 \beta \epsilon)]^{\frac{1}{2}} \mathrm{~V}_1                               \\
  \mathrm{~V}_1
   & =\prod_{\alpha=1}^n e^{0 \mathrm{x}_x}, \quad \tanh \theta \equiv e^{-2 \beta \epsilon}
\end{align}
\begin{align}
  \mathrm{V}_2
   & =\prod_{\alpha=1}^n e^{\beta \epsilon \mathbf{Z}_\alpha \mathbf{Z}_{\alpha+1}}                      \\
  \mathrm{~V}_3
   & =\prod_{\alpha=1}^n e^{\beta B \mathbf{Z}_\alpha} \quad, \quad \mathbf{Z}_{n+1} \equiv \mathbf{Z}_1
\end{align}
\begin{equation}
  \mathrm{P}
  = [2 \sinh (2 \beta \epsilon)]^{\frac{1}{2}} \mathrm{V}_3 \mathrm{V}_2 \mathrm{V}_1
\end{equation}
