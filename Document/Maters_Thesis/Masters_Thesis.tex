...
カラスクイラ(Juan Carrasquilla)とメルコ―(Roger G.Melko)は，2016年の5月に"Machine learning phases of matter"を投稿し，後にNature Physics誌に掲載された．彼らの研究によってはじめてニューラルネットワークを用いて相転移が検出可能であることが示された．ここでは，この論文の内容を引用して，2次元正方イジング模型と2次元三角イジング模型の相転移の検出を行う．\par
\subsection{学習データの作成}
ニューラルネットワークを用いて学習を行うにはまず，モデルの学習のためのデータセットを作成する必要がある．相の分類器の作成には，スピン配位とその配位が秩序相か無秩序相かを示す教師ラベルをセットにした教師ありデータを大量に用意する．\par
この教師データの作成はpythonを用いて次の手順で生成した．
\begin{enumerate}
  \item 温度$T$を選ぶ
  \item メトロポリス法を用いて温度$T$のスピン配位$\bm{\sigma}$をサンプルする
  \item $T<2.27$の場合$d=0$，$T<2.27$の場合$d=0$とする
  \item $(\bm{\sigma},d)$
\end{enumerate}
