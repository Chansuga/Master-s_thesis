...
ニューラルネットワークではユニットと呼ばれる素子からなる．図\ref{ユニットの構造}はユニットの構造を示したものである．見た目はパーセプトロンにおける形式ニューロンと変わらない．しかし，ユニットは形式ニューロンとは違い，0,1の値ではなく，実数値の入出力として持つことができる．また，もう1つの大きな違いは，活性化関数として階段関数ではなく，微分可能は増化関数を採用するということである．例えばシグモイド関数がよく用いられていた．まず，ユニットにさまざまな入力$x_i$は入ってきているとする．各ユニットによって結合強度が異なるため，実際にはそれらの重み付き和
\begin{equation}
  u = \sum_{i} w_i x_i
\end{equation}
