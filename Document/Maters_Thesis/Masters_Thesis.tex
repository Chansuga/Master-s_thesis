...
\begin{center}
  ...
  {\Large\noindent 2024年度 修士論文}\\
  \vspace{50mm}
  {\Huge\noindent\textbf{機械学習と2次元イジング模型}}\\
  \medskip
  {\Huge\noindent\textbf{における相転移検出}}\\
  \vspace{\baselineskip}
  \vspace{50mm}
...
\begin{abstract}
本論文では機械学習による手法を用いた2次元イジング模型の磁気相転移の検出に関する以下の3つの論文についてまとめた．\par
1つ目はカラスクイラとメルコーによる有名な論文である．この論文は機械学習を用いた相転移検出に関する最初の論文であり，シンプルなニューラルネットワークを用いてイジング模型のスピン配位が常磁性相と強磁性相のどちらの属するかを2値分類するモデルを生成して相転移を検出する手法である．\par
2つ目は富谷昭夫氏と田中章詞氏による論文で，こちらもシンプルなニューラルネットワークを用いるが，直接相を分類させるのではなく，温度を分類させるモデルを生成して，その生成したモデルの重みパラメータから相転移を検出するという手法である．この手法はカラスクイラとメルコーによる手法と異なり，モデルに相転移の情報を与えず直接検出可能という利点を持っている．\par
最後に，柏浩二氏，菊池勇太氏，富谷昭夫氏による論文を紹介する．この論文は2つ目の手法について，モデルはスピン配位の情報と温度の情報から何を学んでいるのかを調べ，相転移を検出できる理由を理論的に考察している．
...
\begin{abstract}
This paper summarizes the following three papers on the detection of magnetic phase transitions in two-dimensional Ising models using machine learning methods.\par
The first is the famous paper by Juan Carrasquilla and Roger G.Melko. This is the first paper on the use of machine learning to detect phase transitions, using a simple neural network to generate a binary model that classifies whether the spin configuration of the Ising model belongs to the paramagnetic or ferromagnetic phase.\par
The second paper, by Akio Tomiya and Akinori Tanaka, also uses a simple neural network, but instead of directly classifying phases, it generates a model to classify temperatures and detects phase transitions based on the weight parameters of the Learned model. Unlike the method by Carrasquilla and Melko, this method has the advantage that phase transitions can be detected directly without providing information on the model.\par
Finally, we present a paper by Kouji Kashiwa, Yuta Kikuchi, and Akio Tomiya. This paper investigates what the model learns from the spin configuration and temperature information for the second method, and discusses theoretically why phase transitions can be detected.
...
\chapter{はじめに} %章
\section{研究背景}
近年、急速なデジタル技術の進展と共にAI（人工知能）が様々な分野で注目を集め，その応用範囲が急拡大している．AI技術の進歩は，新たな解決策や知見の発見を促進し，これまで解決が難しかった課題に対するアプローチを変革している．現在このAIブームは，物理学の分野においても大きな影響を与えている．\par
特に物理学の領域では，機械学習やディープラーニングなどのAI技術が新しい研究手法として取り入れられ，研究者たちによって精力的に活用されている．これらの技術は，複雑なデータパターンや相関を検出し，理論的な予測や実験結果の解釈において有益なツールとなっている．物理学者たちは，従来の手法では到達が難しかった理論の深層を探求し，新たな物理的洞察を獲得するために機械学習を利用している．\par
...
本論文ではイジング模型の相転移を扱うが，まずは相転移とは何かを理解するために，物質の三態について説明する．私たちにとって最も身近な相転移現象は水(\ce{H2O})の状態変化であろう．水は固体(氷)，液体(水)，気体(水蒸気)の3つの状態をとる．これら状態には明確な規則性があり，1気圧の環境下であれば，氷点下，つまり摂氏0℃以下のとき水(\ce{H2O})は氷になる．そこから温度を徐々に上げていくと，ちょうど摂氏0℃で氷から水へ状態変化がおき，さらにその水を温めておけば，ちょうど100℃で水から水蒸気への状態変化が起きる．\footnote{そもそも摂氏という温度目盛りは1気圧の環境で水と氷が共存するときを0，水と水蒸気が共存するときを100と定義しているのである．}.\par
...
\subsection{臨界現象とは}
図\ref{三態相図}の相図で液相と気相を仕切る線が消えてしまう点$(T_c,p_c)$を臨界点(critical point)とよぶ．臨界点での温度$T_c$が臨界温度，圧力$p_c$が臨界圧力である．臨界点近傍では，臨界現象(critical phenomena)とよばれる，一連のきわめて興味深い現象がみられる．\par
...
様々な物質について臨界指数$\beta$を調べると，面白いことに，一部の例外を除くほぼすべての物質について
\begin{equation}
  \beta \simeq 0.325
\end{equation}
...
\section{強磁性イジング模型}
これまで，最も身近な相転移という理由で物質の三態を扱ってきが，物理学では物質の三態間の相転移以外にもさまざまな相転移が存在する．その中で現在，理論的な部分で最もよく知られている相転移が強磁性体に関係する相転移である．本論文の目的は機械学習による相転移検出であるが，まずは最も簡単な相転移を扱うのがよいという考えから，本論文ではイジング模型の相転移を扱う．\par
...
\end{equation}
で表せる．和は$2^N$通りのすべてのスピン配位についてとる．後の便利のために格子サイズを$L$とした．物理量$\hat{g}$の期待値は
\begin{equation}
...
\begin{equation}
  E(\bm{\sigma}) = -J \sum_{\langle i,j \rangle} \tilde{\sigma}_i \tilde{\sigma}_j +\mu_0 H \sum_{i=1}^{N} \tilde{\sigma}_i
\end{equation}
のように磁場を反転させた元のエネルギーと同じ表式が得られる．つまり，すべてのスピンを反転させることは，磁場を反転させることと等価ということである．すべての$(\sigma_1,\dots,\sigma_N)$について足し上げることは，すべての$(\tilde{\sigma}_1,\dots,\tilde{\sigma}_N)$について足し上げることと同じなので，分配関数の表式(\ref{イジング分配関数})を，
\begin{align}
  Z_L(\beta, H)
   ...
   & = \sum_{\bm{\sigma}} \exp{\left[ \beta\left( J \sum_{\langle i,j \rangle} \sigma_i \sigma_j -\mu_0 H \sum_{i=1}^{N} \sigma_i \right) \right]} \notag                                         \\
   & = Z_L(\beta, -H)
\end{align}
...
\begin{figure}[H]
  \begin{center}
    \begin{tikzpicture}[scale=1]
      \draw (-5,0) -- (5,0);
      \draw (-5,0) node[left]{$\cdots$};
      \draw (5,0) node[right]{$\cdots$};
      \filldraw[fill=white] (-4,0) circle[radius=0.25];
      \filldraw[fill=white] (-2,0) circle[radius=0.25];
      \filldraw[fill=white] (0,0) circle[radius=0.25];
      \filldraw[fill=white] (2,0) circle[radius=0.25];
      \filldraw[fill=white] (4,0) circle[radius=0.25];
      \draw (-4,-0.4) node[below]{$\sigma_{i-2}$};
      \draw (-2,-0.4) node[below]{$\sigma_{i-1}$};
      \draw (0,-0.4) node[below]{$\sigma_i$};
      \draw (2,-0.4) node[below]{$\sigma_{i+1}$};
      \draw (4,-0.4) node[below]{$\sigma_{i+2}$};
    \end{tikzpicture}
  ...
  \end{center}
\end{figure}
これまでの議論は次元に依存しない部分を扱った．ここからは次元に分けて説明する．まず最も簡単な1次元のイジング模型を考える．図\ref{1次元イジング模型格子構造}は1次元イジング模型の格子構造で，スピンが等間隔で$L$個1列に並んでいる構造をしている．ハミルトニアンは
\begin{equation}
  \mathcal{H} = -J \sum_{i=1}^L \sigma_i \sigma_j - H \sum_{i=1}^L \sigma_i \label{1次元イジングエネルギー}
\end{equation}
で与えられる．ただし，簡単のため$\mu_0=1$とした．また，ここでは周期的境界条件として$\sigma_{L+1} = \sigma_{1}$を課す．このとき分配関数は
\begin{equation}
  Z = \sum_{(\sigma_i,\dots,\sigma_L)} \exp{\left[ \beta J \sum_{i=1}^L \sigma_i \sigma_j + \beta H \sum_{i=1}^L \sigma_i \right]} \label{1次元イジング分配関数}
\end{equation}
...
\begin{align}
  Z
   & = \sum_{(\sigma_i,\dots,\sigma_L)} (\mathrm{T})_{\sigma_1, \sigma_2} (\mathrm{T})_{\sigma_2, \sigma_3} \cdots (\mathrm{T})_{\sigma_L, \sigma_1} \notag \\
   & = \sum_{(\sigma_i,\dots,\sigma_L)} \prod_{i = 1}^L (\mathrm{T})_{\sigma_i, \sigma_{i+1}}
\end{align}
を書くことができる．行列の積の定義より，
\begin{equation}
...
\begin{align}
  Z
   & = \sum_{\sigma_1 = \pm{1}}\sum_{\sigma_2 = \pm{1}} \cdots \sum_{\sigma_L = \pm{1}} (\mathrm{T})_{\sigma_1, \sigma_2} (\mathrm{T})_{\sigma_2, \sigma_3} \cdots (\mathrm{T})_{\sigma_L, \sigma_1} \notag   \\
   & = \sum_{\sigma_1 = \pm{1}}\sum_{\sigma_3 = \pm{1}} \cdots \sum_{\sigma_L = \pm{1}} (\mathrm{T}^2)_{\sigma_1, \sigma_3} (\mathrm{T})_{\sigma_3, \sigma_4} \cdots (\mathrm{T})_{\sigma_L, \sigma_1} \notag \\
   & = \sum_{\sigma_1 = \pm{1}}\sum_{\sigma_4 = \pm{1}} \cdots \sum_{\sigma_L = \pm{1}} (\mathrm{T}^3)_{\sigma_1, \sigma_4} (\mathrm{T})_{\sigma_4, \sigma_5} \cdots (\mathrm{T})_{\sigma_L, \sigma_1} \notag \\
   & = \cdots \notag                                                                                                                                                                                          \\
   & = \sum_{\sigma_1 = \pm{1}} (\mathrm{T}^L)_{\sigma_1, \sigma_1} \notag                                                                                                                                    \\
   & = \mathrm{Tr}\left[ \mathrm{T}^L \right] \label{1Dイジング分配関数トレース}
\end{align}
...
\begin{equation}
  \lambda_{\pm{1}}
  = e^{\beta J} \left\{ \cosh{\beta h} \pm{\sqrt{(\sinh{\beta h})^2 + e^{-4\beta J}}} \right\}
\end{equation}
...
\begin{align}
  ...
  \begin{pmatrix}
      \lambda_{+} & 0           \\
      0           & \lambda_{-}
    \end{pmatrix} \mathrm{O}^{-1} \right\}^L \right] \notag   \\
   & = \mathrm{Tr} \left[
  \begin{pmatrix}
      \lambda_{+} & 0           \\
      0           & \lambda_{-}
    \end{pmatrix}^L \right] \notag                            \\
   & = (\lambda_{+})^L + (\lambda_{-})^L \label{1Dイジング分配関数}
\end{align}
と表すことができる．\par
...
\begin{align}
  f
   & = -\frac{1}{\beta L} \ln{Z} \notag                                                                                              \\
   & = -\frac{1}{\beta L} \ln{(\lambda_{+})^L + (\lambda_{-})^L} \notag                                                              \\
...
\begin{align}
  f
   ...
   & = -\frac{1}{\beta} \ln{e^{\beta J} \left\{ \cosh{\beta H} \pm{\sqrt{(\sinh{\beta h})^2 + e^{-4\beta J}}}\right\}} \notag    \\
...
\begin{figure}[H]
  \begin{center}
    \begin{tikzpicture}[scale=0.75]
      ...
      \draw ( 3, 1.73) -- ( 5,-1.73);
      \draw ( 5, 1.73) -- ( 6, 0);
      \filldraw[fill=white] (-6,0) circle[radius=0.25];
      \filldraw[fill=white] (-4,0) circle[radius=0.25];
      \filldraw[fill=white] (-2,0) circle[radius=0.25];
      \filldraw[fill=white] (-6,2) circle[radius=0.25];
      \filldraw[fill=white] (-4,2) circle[radius=0.25];
      \filldraw[fill=white] (-2,2) circle[radius=0.25];
      \filldraw[fill=white] (-6,-2) circle[radius=0.25];
      \filldraw[fill=white] (-4,-2) circle[radius=0.25];
      \filldraw[fill=white] (-2,-2) circle[radius=0.25];
      \filldraw[fill=white] (2,0) circle[radius=0.25];
      \filldraw[fill=white] (4,0) circle[radius=0.25];
      \filldraw[fill=white] (6,0) circle[radius=0.25];
      \filldraw[fill=white] (3,1.73) circle[radius=0.25];
      \filldraw[fill=white] (5,1.73) circle[radius=0.25];
      \filldraw[fill=white] (7,1.73) circle[radius=0.25];
      \filldraw[fill=white] (1,-1.73) circle[radius=0.25];
      \filldraw[fill=white] (3,-1.73) circle[radius=0.25];
      \filldraw[fill=white] (5,-1.73) circle[radius=0.25];
    \end{tikzpicture}
    \caption{2次元イジング模型の代表的な格子構造．左は正方イジング模型または，単にイジング模型とよばれ，右は三角イジング模型とよばれる．}
    \label{2次元イジング模型格子構造}
  \end{center}
\end{figure}
次に，2次元イジング模型を考える．2次元イジング模型は2次元平面状に$N(=L \times L)$個の格子点が等間隔に配置された模型である．図\ref{2次元イジング模型格子構造}は2次元イジング模型の代表的な格子構造である．左は正方イジング模型とよばれるもので最も基本的な格子構造である．一般的に2次元イジング模型と書かれたら，正方イジング模型のことを指す．そのため本論文でも正方イジング模型を単にイジング模型とよぶことにする．右は三角イジング模型とよばれる．\par
ハミルトニアンは
\begin{equation}
  \mathcal{H} = - \sum_{i=1}^{N} \sum_{j \in \mathcal{N}(i)} \sigma_i \sigma_j
...
順伝播型ニューラルネットワークとはこのようなユニットを層状につなぎ合わせたアーキテクチャで，図\ref{順伝播ニューラルネットワークの例}はその典型的な一例である\footnote{本論文ではニューラルネットワークのユニット同士をつなぐ線を矢印ではなく，線で表すことにする．}．$l$は層数で，$l=0$が入力層，$l=3$が出力層，そして，$l=1,2$が中間層を表している．信号の伝達は$l=0,1,2,3$の順に進み，そのため順伝播型と呼ばれる．\par
...
%   \begin{minipage}[b]{0.45\linewidth}
%     \begin{center}
%       \includegraphics[keepaspectratio, scale=0.2]{image/温度測定器2_L16_FCN_Nh3_mag.png}
%       \subcaption{磁化との相関}
%       \label{温度測定器2磁化との相関}
%     \end{center}
%   \end{minipage}
%   \begin{minipage}[b]{0.45\linewidth}
%     \begin{center}
%       \includegraphics[keepaspectratio, scale=]{image/温度測定器2_FCN_modeling.png}
%       \subcaption{直線の式でモデリング}
%     \end{center}
%   \end{minipage}
% \end{figure}
